...
private:
    ...
    QString toGanZhiYear(int lYear) {
        int ganKey = (lYear + 5) % 10;
        int zhiKey = (lYear + 3) % 12;
        return 天干[ganKey] + 地支[zhiKey];
    }

    QString toGanZhiMonth(int lYear, int lMonth)
    {
        return 天干[ ((lYear + 5) % 10 % 5 * 2 + 2 + (lMonth - 1)) % 10] + 地支[((lMonth - 1) + 2) % 12];
    }

    QString toGanZhiDay(int offset)
    {
        return 天干[offset%10] + 地支[offset%12];
    }

    QString toGanZhiHour(int offset, int hour)
    {
        return 天干[ offset % 10 % 5 * 2] + 地支[(hour + 1) / 2 % 12];
    }
